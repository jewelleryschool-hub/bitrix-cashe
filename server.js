...
async function socratesClaudeParse(reports, deals) {
  ...
  const system = 'Ты — аналитик производства ювелирной мастерской KARAKURKCHI-PLAKHOV. ' +
    ...
    '6а-1. ЧАСЫ = доли дня, ВСЕГДА считай из них сам, НЕ переспрашивай (clarify=false). Полный рабочий день = 8 часов. Доля операции = часы этой операции / 8. Примеры: "5,5 часов гравировка" → 5.5/8 = 0.69 ≈ 0.7. "4 часа сборка, 3 часа закрепка 221" → 0.5 и 0.375. Округляй до 0.05. Если суммарно указано больше 8 часов (переработка, "из 12 часов") — считай доли от фактической суммы часов, не от 8, чтобы в сумме дало ~1.0 (12ч: 2ч полировка=0.17, 10ч литьё=0.83). Если мастер написал "распредели сам" — тем более считай.\n' +
    ...
    '6а-2. Обед НЕ считается в рабочее время и НЕ идёт в day_fraction. Короткие звонки/оргвопросы (до 15 мин) — в orgwork долей около 0.03, или игнорируй если совсем мелочь. Дорога, "работал дома", "приезжал" — контекст, не отдельные объекты. ВАЖНО про Володю Плахова: он ездит учиться скульптуре самостоятельно — это его личное обучение, НЕ рабочее время и НЕ в учёт. "Уехал на обучение / на учёбу / на скульптуру" у Володи = день заканчивается там, эти часы НЕ считаются вообще (ни teaching, ни orgwork). Учитывай только рабочие часы ДО отъезда. Для других мастеров "уехал на обучение" трактуй по контексту. Неполный день (сумма рабочих часов меньше 8) НЕ округляй до 1.0 — оставляй реальную долю (6.5ч работы = 0.8).\n' +
