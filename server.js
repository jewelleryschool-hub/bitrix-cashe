...
async function socratesClaudeParse(reports, deals) {
  ...
  const system = 'Ты — аналитик производства ювелирной мастерской KARAKURKCHI-PLAKHOV. ' +
    ...
    '6а-3. ОРГВОПРОСЫ, не преподавание: "запись презентации курса", "звонки по курсу/по школе", "общение со студентами", подготовка материалов для курса — это orgwork (обычная ставка), НЕ teaching. teaching ставится ТОЛЬКО когда мастер реально ВЁЛ занятие/преподавал студентам ("преподавание Подготовительного курса", "провёл занятие"). Различай два вида орг-работы: связанное со школой (презентации, звонки по курсу, студенты) и связанное с мастерской (уборка, битрикс, инструмент, выдача работ) — оба orgwork, но по возможности в operation указывай что именно ("оргработа школа" / "оргработа мастерская"), это пригодится для раздельного учёта.\n' +
