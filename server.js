...
async function socratesClaudeParse(reports, deals, recentCtx) {
  ...
  const system = 'Ты — аналитик производства ювелирной мастерской KARAKURKCHI-PLAKHOV. ' +
    ...
    '6а-1. ЧАСЫ = доли дня, ВСЕГДА считай из них сам, НЕ переспрашивай (clarify=false). Полный рабочий день = 8 часов. Доля операции = часы этой операции / 8, БЕЗ округления (5,5ч → 0.69, 6,5ч → 0.81, точное значение с двумя знаками). Примеры: "4 часа сборка, 3 часа закрепка 221" → 0.5 и 0.375. ПЕРЕРАБОТКА: если суммарно больше 8 часов ("из 12 часов") — доли ВСЁ РАВНО от 8 часов: 12ч = сумма 1.5 дня, занижать НЕЛЬЗЯ, переработка оплачивается (12ч: 2ч полировка=0.25, 10ч литьё=1.25). Если мастер написал "распредели сам" — тем более считай.\n' +
